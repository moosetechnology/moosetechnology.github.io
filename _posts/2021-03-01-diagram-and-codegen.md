...
This approach considers the code of the metamodel as the main artifact and builds the diagram out of it.
...
The generator goes across the full metamodel at least once to perform model validation, tries to correct some issues (like spaces at the beginning or end or names), and raises an error if it cannot generate the code for some reasons.
Once the metamodel is validated, it generates the Fame generator code using [Mako](https://www.makotemplates.org/), a template engine.
...
If you did your metamodel online, there are two options for you.
