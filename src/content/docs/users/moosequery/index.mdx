...
- All dependencies to and from Class1:
    ...
    class1 query outgoing dependencies. "=> { access1 } This is here because Class1 contains Method1 that access AttributeA and queries are recursive by default"
    class1 query outgoing objects dependencies. "=> { attributeA }"
    ```

- Local dependencies to Class1:
    As we have seen, asking the outgoing dependencies of `Class1` gives `AttributeA` because it is accessed in `Method1` that is contained in `Class1`.
